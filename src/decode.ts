 ...
 * Matches IRremoteESP8266's global `kMarkExcess` (50µs) so real-hardware
 * captures decode identically to the C++ reference. Ideal encoder output also
 * matches, since exact timings stay within the shifted tolerance windows.
 */
export const kMarkExcess = 50;

/** Alias of {@link kMarkExcess}; kept for call sites that pass it explicitly. */
...
import { decodeWhirlpoolMagicool2 } from "./protocols/whirlpool_magicool2.js";
import type { WhirlpoolMagicool2State } from "./protocols/whirlpool_magicool2.js";
...
import { decodeFujitsu } from "./protocols/fujitsu.js";
...
export type DecodeResult =
  ...
  | { protocol: "whirlpool_magicool2"; brand: "whirlpool"; type: "ac"; state: WhirlpoolMagicool2State; confidence: "checksum_valid" }
  ...
  | { protocol: "fujitsu_ac"; brand: "fujitsu"; type: "ac"; state: FujitsuState; confidence: "checksum_valid" }
...
const PROTOCOL_REGISTRY: ProtocolEntry[] = [
  ...
  {
    protocol: "whirlpool_magicool2", brand: "whirlpool", type: "ac",
    tryDecode(timings, offset, ho) {
      const s = decodeWhirlpoolMagicool2(timings, offset, ho);
      return s ? { protocol: "whirlpool_magicool2", brand: "whirlpool", type: "ac", state: s, confidence: "checksum_valid" } : null;
    },
  },
  {
  ...
  {
    protocol: "goodweather", brand: "goodweather", type: "ac",
    tryDecode(timings, offset, ho) {
      const s = decodeGoodweather(timings, offset, ho);
      return s ? { protocol: "goodweather", brand: "goodweather", type: "ac", state: s, confidence: "checksum_valid" } : null;
    },
  },
  {
    protocol: "transcold", brand: "transcold", type: "ac",
    tryDecode(timings, offset, ho) {
  ...
  {
    protocol: "fujitsu_ac", brand: "fujitsu", type: "ac",
    tryDecode(timings, offset, ho) {
      const s = decodeFujitsu(timings, offset, ho);
      return s ? { protocol: "fujitsu_ac", brand: "fujitsu", type: "ac", state: s, confidence: "checksum_valid" } : null;
    },
  },
  {
